...
export function CommandeDetailModal({ commande, isOpen, onClose, onSuccess }) {
  ...
  const { data: articles = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get(API_ENDPOINTS.ARTICLES, {
      ...
      const responses = await Promise.all(
        articlesNS.map((codeArticle) =>
          apiClient.get("/api/stock/unites-article/", {
            params: { article: codeArticle, statut: "EN_STOCK", page_size: 100 },
          })
        )
      );
      const unites = {};
      responses.forEach((res, idx) => {
        unites[articlesNS[idx]] = res.data.results ?? res.data;
      });
      return unites;
    },
  ...
  const traiterMutation = useMutation({
    mutationFn: async ({ commandeId, payload }) => {
      const { data } = await apiClient.post(
        `${API_ENDPOINTS.COMMANDES}${commandeId}/traiter/`,
        payload
      );
      return data;
    },
    ...
    onError: (err) => {
      ...
      if (detail && typeof detail === "object") {
        ...
        notify.error("Erreur lors du traitement de la commande.");
      }
    },
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
          <Typography variant="h3">
            Commande <CodeChip value={`#${commande.commande_id}`} />
          </Typography>
