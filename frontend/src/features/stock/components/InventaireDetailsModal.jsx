...
import { Dialog, DialogTitle, DialogContent, DialogActions, Button, IconButton, Box, Typography, Divider, CircularProgress } from "@mui/material";
import { Close as CloseIcon, CheckCircle as CheckCircleIcon, Inventory as InventoryIcon } from "@mui/icons-material";
...
export function InventaireDetailsModal({ session, isOpen, onClose, onSuccess }) {
  ...
  const validerMutation = useMutation({
    ...
    onError: (err) => {
      ...
      if (detail && typeof detail === "object") {
        notify.error(Object.entries(detail).map(([k, v]) => `${k}: ${Array.isArray(v) ? v.join(", ") : v}`).join(" | "));
      } else {
  ...
  const handleValider = () => {
    confirm(
      ...
      "Voulez-vous vraimment valider cette inventaire ? Cette action est irréversible. Continuer ?",
      async () => {
        setValidating(true);
    ...
    <Dialog open={isOpen} onClose={onClose} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: 2 } }}>
      <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", bgcolor: "#FFF8E1", borderBottom: "2px solid", borderColor: "primary.main" }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
          <Typography variant="h3">Inventaire <CodeChip value={session.code_reference} /></Typography>
      ...
      <DialogContent sx={{ pt: 3 }}>
        ...
        <InventaireStatsCards nbArticles={nbArticles} nbEcarts={nbEcarts} ecartTotal={ecartTotal} />
        
        <Divider sx={{ mb: 2 }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, color: "text.secondary" }}>
            <InventoryIcon fontSize="small" />
            <Typography variant="body2" fontWeight={600}>Détails des articles</Typography>
          </Box>
        </Divider>
      ...
      <DialogActions sx={{ px: 3, pb: 2 }}>
        <Button onClick={onClose} disabled={validating}>Fermer</Button>
        {peutValider && (
          <Button
            variant="contained"
            color="success"
